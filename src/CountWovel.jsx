...
export default function CountWovel() {
  ...
  const name6 = "mohammed";
  const Wovel6 = ["o", "e", "d"];
  function countWovel6(word) {
    const newWord = word.split("");
    console.log(newWord);
    const result = [];
    for (let char of newWord) {
      if (Wovel6.includes(char)) {
        result.push(char);
      }
    }
    console.log(result.length);
  ...
  const name1 = "faizan";
  let name1Wovel = ["f", "a", "z"];
  function findName1Wovel(word) {
    let result = [];
    for (let char of word) {
      if (name1Wovel.includes(char) && !result.includes(char)) {
        result.push(char);
      }
    }
    console.log(result.length);
